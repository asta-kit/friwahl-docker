 ...
 */
class BallotBoxRepository_Original extends Repository {
 ...
 */
class BallotBoxRepository extends BallotBoxRepository_Original implements \TYPO3\Flow\Object\Proxy\ProxyInterface {
	...
	public function __construct() {
		if (get_class($this) === 'AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository') \TYPO3\Flow\Core\Bootstrap::$staticObjectManager->setInstance('AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository', $this);
		parent::__construct();
		if ('AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository' === get_class($this)) {
	...
	$reflectedClass = new \ReflectionClass('AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository');
	$allReflectedProperties = $reflectedClass->getProperties();
	foreach ($allReflectedProperties as $reflectionProperty) {
		$propertyName = $reflectionProperty->name;
		if (in_array($propertyName, array('Flow_Aop_Proxy_targetMethodsAndGroupedAdvices', 'Flow_Aop_Proxy_groupedAdviceChains', 'Flow_Aop_Proxy_methodIsInAdviceMode'))) continue;
		if (isset($this->Flow_Injected_Properties) && is_array($this->Flow_Injected_Properties) && in_array($propertyName, $this->Flow_Injected_Properties)) continue;
		if ($reflectionProperty->isStatic() || $reflectionService->isPropertyAnnotatedWith('AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository', $propertyName, 'TYPO3\Flow\Annotations\Transient')) continue;
		...
		if (is_object($this->$propertyName) && !$this->$propertyName instanceof \Doctrine\Common\Collections\Collection) {
			if ($this->$propertyName instanceof \Doctrine\ORM\Proxy\Proxy) {
				...
				$varTagValues = $reflectionService->getPropertyTagValues('AstaKit\FriWahl\Core\Domain\Repository\BallotBoxRepository', $propertyName, 'var');
