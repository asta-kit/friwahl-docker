<?php
# PackageStates.php

# This file is maintained by Flow's package management. Although you can edit it
# manually, you should rather use the command line commands for maintaining packages.
# You'll find detailed information about the typo3.flow:package:* commands in their
# respective help screens.

# This file will be regenerated automatically if it doesn't exist. Deleting this file
# should, however, never become necessary if you use the package commands.

return array (
  'packages' => 
  array (
    'doctrine.lexer' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/lexer',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/lexer/',
      'classesPath' => 'lib/',
    ),
    'doctrine.annotations' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/annotations',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/annotations/',
      'classesPath' => 'lib/',
    ),
    'doctrine.cache' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/cache',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/cache/',
      'classesPath' => 'lib/',
    ),
    'doctrine.collections' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/collections',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/collections/',
      'classesPath' => 'lib/',
    ),
    'doctrine.inflector' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/inflector',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/inflector/',
      'classesPath' => 'lib/',
    ),
    'Doctrine.Common' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/common',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/common/',
      'classesPath' => 'lib/',
    ),
    'Doctrine.DBAL' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/dbal',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/dbal/',
      'classesPath' => 'lib/',
    ),
    'doctrine.migrations' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/migrations',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/migrations/',
      'classesPath' => 'lib/',
    ),
    'symfony.polyfillmbstring' => 
    array (
      'manifestPath' => '',
      'composerName' => 'symfony/polyfill-mbstring',
      'state' => 'active',
      'packagePath' => 'Libraries/symfony/polyfill-mbstring/',
      'classesPath' => 'Classes/',
    ),
    'psr.log' => 
    array (
      'manifestPath' => '',
      'composerName' => 'psr/log',
      'state' => 'active',
      'packagePath' => 'Libraries/psr/log/',
      'classesPath' => 'Classes/',
    ),
    'symfony.debug' => 
    array (
      'manifestPath' => '',
      'composerName' => 'symfony/debug',
      'state' => 'active',
      'packagePath' => 'Libraries/symfony/debug/',
      'classesPath' => 'Classes/',
    ),
    'symfony.console' => 
    array (
      'manifestPath' => '',
      'composerName' => 'symfony/console',
      'state' => 'active',
      'packagePath' => 'Libraries/symfony/console/',
      'classesPath' => 'Classes/',
    ),
    'Doctrine.ORM' => 
    array (
      'manifestPath' => '',
      'composerName' => 'doctrine/orm',
      'state' => 'active',
      'packagePath' => 'Libraries/doctrine/orm/',
      'classesPath' => 'lib/',
    ),
    'composer.installers' => 
    array (
      'manifestPath' => '',
      'composerName' => 'composer/installers',
      'state' => 'active',
      'packagePath' => 'Libraries/composer/installers/',
      'classesPath' => 'Classes/',
    ),
    'symfony.domcrawler' => 
    array (
      'manifestPath' => 'Symfony/Component/DomCrawler/',
      'composerName' => 'symfony/dom-crawler',
      'state' => 'active',
      'packagePath' => 'Libraries/symfony/dom-crawler/',
      'classesPath' => '',
    ),
    'symfony.yaml' => 
    array (
      'manifestPath' => 'Symfony/Component/Yaml/',
      'composerName' => 'symfony/yaml',
      'state' => 'active',
      'packagePath' => 'Libraries/symfony/yaml/',
      'classesPath' => '',
    ),
    'mikey179.vfsStream' => 
    array (
      'manifestPath' => '',
      'composerName' => 'mikey179/vfsStream',
      'state' => 'active',
      'packagePath' => 'Libraries/mikey179/vfsStream/',
      'classesPath' => 'src/main/php/',
    ),
    'TYPO3.Fluid' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/fluid',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Fluid/',
      'classesPath' => 'Classes/',
    ),
    'TYPO3.Eel' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/eel',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Eel/',
      'classesPath' => 'Classes/',
    ),
    'TYPO3.Flow' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/flow',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Flow/',
      'classesPath' => 'Classes/',
    ),
    'TYPO3.Party' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/party',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Party/',
      'classesPath' => 'Classes/',
    ),
    'TYPO3.Kickstart' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/kickstart',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Kickstart/',
      'classesPath' => 'Classes/',
    ),
    'TYPO3.Welcome' => 
    array (
      'manifestPath' => '',
      'composerName' => 'typo3/welcome',
      'state' => 'active',
      'packagePath' => 'Framework/TYPO3.Welcome/',
      'classesPath' => 'Classes/',
    ),
    'AstaKit.FriWahl.AdminInterface' => 
    array (
      'manifestPath' => '',
      'composerName' => 'astakit/friwahl-admininterface',
      'state' => 'active',
      'packagePath' => 'Application/AstaKit.FriWahl.AdminInterface/',
      'classesPath' => 'Classes/',
    ),
    'AstaKit.FriWahl.BallotBoxBackend' => 
    array (
      'manifestPath' => '',
      'composerName' => 'astakit/friwahl-ballotboxbackend',
      'state' => 'active',
      'packagePath' => 'Application/AstaKit.FriWahl.BallotBoxBackend/',
      'classesPath' => 'Classes/',
    ),
    'AstaKit.FriWahl.Core' => 
    array (
      'manifestPath' => '',
      'composerName' => 'astakit/friwahl-core',
      'state' => 'active',
      'packagePath' => 'Application/AstaKit.FriWahl.Core/',
      'classesPath' => 'Classes/',
    ),
    'AstaKit.FriWahl.VoterDirectory' => 
    array (
      'manifestPath' => '',
      'composerName' => 'astakit/friwahl-voterdirectory',
      'state' => 'active',
      'packagePath' => 'Application/AstaKit.FriWahl.VoterDirectory/',
      'classesPath' => 'Classes/',
    ),
  ),
  'version' => 4,
);